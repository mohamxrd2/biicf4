    ...
    <div class="bg-white dark:bg-neutral-800 rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="relative px-6 sm:px-8 -mt-20">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-end space-y-4 sm:space-y-0 sm:space-x-6">
                <form action="{{ route('biicf.updateProfilePhoto', ['user' => $user->id]) }}" 
                    method="post" enctype="multipart/form-data" id="photo-upload-form">
                    @csrf
                    @method('PUT')
                    <div class="relative">
                        <div class="w-32 h-32 rounded-2xl overflow-hidden ring-4 ring-white dark:ring-neutral-700 shadow-lg">
                            <img id="img" src="{{ asset($user->photo) }}" 
                ...
                <div id="basic-tabs-1" role="tabpanel" aria-labelledby="basic-tabs-item-1">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @php
                            ...
                            if($user->country) $userInfo[] = ['label' => 'Pays', 'value' => $user->country, 'icon' => 'globe'];
                            if($user->local_area) $userInfo[] = ['label' => 'Localité', 'value' => $user->local_area, 'icon' => 'location-marker'];
                            if($user->address) $userInfo[] = ['label' => 'Adresse', 'value' => $user->address, 'icon' => 'home'];
                            if($user->active_zone) $userInfo[] = ['label' => 'Zone d\'activité', 'value' => $user->active_zone, 'icon' => 'map'];
                            if($user->actor_type) $userInfo[] = ['label' => 'Type d\'acteur', 'value' => $user->actor_type, 'icon' => 'user-group'];
                            if($user->gender) $userInfo[] = ['label' => 'Sexe', 'value' => $user->gender, 'icon' => 'user-circle'];
                            if($user->service_type) $userInfo[] = ['label' => 'Type de service', 'value' => $user->service_type, 'icon' => 'briefcase'];
                            if($user->sector) $userInfo[] = ['label' => 'Secteur d\'activité', 'value' => $user->sector, 'icon' => 'office-building'];
                        @endphp
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M{{ $info['icon'] }}" />
                                        </svg>
                                    </div>
                                    <div>
