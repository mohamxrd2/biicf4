<div class="p-6 bg-gray-100 border rounded-lg shadow-sm max-w-md">
    ...
    <div x-data="{ hasTime: false, hasPeriod: false }">
        <!-- Heure de retrait -->
        <div class="mb-4" x-show="!hasPeriod">
            <label for="{{ $timeId }}" class="block text-sm font-medium text-gray-700 mb-2">Heure de
                retrait</label>
            <div class="relative">
                <input type="time" id="{{ $timeId }}" name="{{ $timeModel }}"
                    wire:model="{{ $timeModel }}"
                    class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    @change="hasTime = $event.target.value !== ''; hasPeriod = false">
            </div>
        </div>

        <!-- Période -->
        <div x-show="!hasTime">
            <label for="{{ $periodId }}" class="block text-sm font-medium text-gray-700 mb-2">Période de
                retrait</label>
            <select id="{{ $periodId }}" name="{{ $periodModel }}" wire:model="{{ $periodModel }}"
                ...
                @change="hasPeriod = $event.target.value !== ''; hasTime = false">
