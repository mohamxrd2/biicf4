    ...
    <section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
            <div class="mx-auto max-w-5xl">
                ...
                <div class="mt-6 flow-root sm:mt-8">
                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @foreach ($notifications as $notification)
                            @php
                                // Récupérer les informations utilisateur et crédit associées
                                $userId = $notification->data['user_id'];
                                $userDetails = App\Models\User::find($userId);

                                $userNumber = $userDetails ? $userDetails->phone : null;
                                $userInPromir = $userNumber
                                    ? App\Models\UserPromir::where('numero', $userNumber)->first()
                                    : null;

                                $crediScore = $userInPromir
                                    ? App\Models\CrediScore::where('id_user', $userInPromir->id)->first()
                                    : null;
                                $demandeId = $notification->data['demande_id'];
                                $demandeCredit = App\Models\DemandeCredi::where('demande_id', $demandeId)->first();

                                // Récupérer le projet associé avec id_projet
                                $projetId = $notification->data['id_projet'];
                                if ($projetId) {
                            ...
                            <div class="flex flex-wrap items-center gap-y-4 py-6">
                                @if ($notification->data['id_projet'])
                                    <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                        <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Projet ID:
                                        </dt>
                                        <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
                                            <a href="#"
                                                class="hover:underline">#{{ $notification->data['id_projet'] }}</a>
                                        </dd>
                                    </dl>
                                @endif
                                @if ($notification->data['demande_id'])
                                    <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                        <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Demande ID:
                                        </dt>
                                        <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
                                            <a href="#"
                                ...
                                @if ($demandeCredit)
                                    <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                        <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Date fin:
                                        </dt>
                                        <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
                ...
                @if ($notification->reponse == 'approved') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                ...
                @else 
                    bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300 @endif">
                                        ...
                                        {{ ucfirst($notification->reponse == 'approved' ? 'confirmer' : ($notification->reponse == 'refuser' ? 'refuser' : 'En Attente')) }}
                                    </dd>
                                </dl>
