                ...
                <div class="lg:w-2/3">
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="relative">
                            ...
                            {{-- Carousel amélioré --}}
                            <div class="carousel relative overflow-hidden aspect-video bg-gray-100 rounded-t-2xl">
                                <div class="carousel-inner relative w-full h-full">
                                    @if (count($photos) > 0)
                                        @foreach ($photos as $index => $photo)
                                            <div class="carousel-item absolute w-full h-full transition-opacity duration-500 ease-in-out {{ $index === 0 ? 'active' : 'hidden' }}"
                                                data-index="{{ $index }}">
                                                <img src="{{ asset('post/all/' . $photo) }}"
                                    ...
                                    @else
                                        <div class="w-full h-full flex items-center justify-center">
                                            <img src="{{ asset('img/noimg.jpeg') }}" class="w-full h-full object-cover"
                                ...
                                @if (count($photos) > 1)
                                    ...
                                    <button type="button" id="carousel-prev"
                                        class="carousel-prev absolute inset-y-0 left-0 flex items-center justify-center w-12 h-full hover:bg-black/20 transition-colors">
                                    ...
                                    <button type="button" id="carousel-next"
                                        class="carousel-next absolute inset-y-0 right-0 flex items-center justify-center w-12 h-full hover:bg-black/20 transition-colors">
                                    ...
                                    <div class="absolute bottom-4 left-0 right-0 flex justify-center space-x-2">
                                        @foreach ($photos as $index => $photo)
                                            <span
                                                class="w-2.5 h-2.5 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-white/50' }} transition-colors cursor-pointer"
                        ...
                        @if (count($photos) > 1)
                            <div class="p-4 bg-white">
                                <div class="flex overflow-x-auto space-x-2 py-2">
                                    @foreach ($photos as $index => $photo)
                                        <div class="flex-shrink-0 cursor-pointer thumbnail-item"
                                            data-index="{{ $index }}">
                                            <img src="{{ asset('post/all/' . $photo) }}"
                                                class="h-16 w-16 object-cover rounded-lg {{ $index === 0 ? 'ring-2 ring-blue-500' : '' }}"
                    ...
                    <div class="mt-6 bg-white rounded-2xl shadow-lg overflow-hidden p-6">
                        ...
                        <p class="text-gray-700">
                            {{ $produits->description ?? 'Aucune description disponible pour ce produit.' }}
                        </p>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="p-6 space-y-4">
                            @if ($produits->statuts === 'Accepté')
                                <div
                                    class="flex items-center justify-center px-4 py-3 bg-green-50 text-green-700 rounded-xl">
                            ...
                            @if ($produits->statuts === 'Refusé')
                                <div
                                    class="flex items-center justify-center px-4 py-3 bg-red-50 text-red-700 rounded-xl">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="bg-gray-50 p-4 rounded-xl">
                                        ...
                                        <p class="text-green-600 font-semibold">
                                            {{ number_format($produits->prix, 0, ',', ' ') }} FCFA</p>
                                    </div>
                                </div>
                                ...
                                @if ($produits->qteProd_min || $produits->qteProd_max)
                                    <div class="bg-gray-50 p-4 rounded-xl">
                                        ...
                                        <p class="text-gray-900">[ {{ $produits->qteProd_min }} -
                                            {{ $produits->qteProd_max }} ]</p>
                                    </div>
                                @endif
                                ...
                                <div class="bg-gray-50 p-4 rounded-xl">
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex items-center">
                                            ...
                                            <span
                                                class="text-gray-900">{{ $produits->villeServ }}{{ $produits->comnServ ? ', ' . $produits->comnServ : '' }}</span>
                                        </div>
