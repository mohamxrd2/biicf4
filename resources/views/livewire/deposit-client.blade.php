        ...
        x-effect="roi = Math.round((amount * 1.10) / 5) * 5">
            ...
            <template x-if="deposit_type === 'Rechargement SOS'">
                <form wire:submit.prevent="submitSOSRecharge">
                    ...
                    <div class="mb-4">
                        {{-- <label for="roi">Montant à envoyer (avec 1%)</label>
                        <input type="number" 
                            wire:model="roi" 
                            id="roi"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm 
                        ...
                        <p class="text-sm text-gray-600 mt-1">
                            Montant à envoyer (avec 1%)
                            <span class="font-bold text-gray-900"> {{ $roi ? number_format($roi, 0, ',', ' ') . ' FCFA' : '0 FCFA' }}</span>
                    ...
                    <div class="mt-6 relative">
                        <button type="submit"
                            class="w-full py-3 bg-purple-600 hover:bg-purple-700 transition-colors 
                                rounded-md text-white font-medium"
                            wire:loading.attr="disabled">
                            <span wire:loading.remove>Soumettre</span>
                            <span wire:loading wire:target="submitSOSRecharge">
